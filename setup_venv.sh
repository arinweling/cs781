...
echo "[7/8] Cloning VerifAI repository if missing..."
if [ ! -d "$ROOT_DIR/VerifAI/.git" ]; then
  git clone https://github.com/BerkeleyLearnVerify/VerifAI "$ROOT_DIR/VerifAI"
else
  echo "VerifAI repository already present. Skipping clone."
fi

echo "[8/8] Performing editable install (pip install -e .)..."
cd "$ROOT_DIR/VerifAI"
